...
- **메시지 검색** - PostgreSQL Full-Text Search (tsvector + GIN) + 커서 기반 페이지네이션
...
| **Message** | 메시지 (CHAT / SYSTEM / FILE, soft delete, tsvector FTS) |
...
| 메시지 검색 | PostgreSQL tsvector + GIN (simple config) | 별도 인프라 없이 FTS 가능, 한국어 한계는 의도적 수용 |
| 검색 페이지네이션 | 커서 기반 (size+1 패턴) | offset 기반 대비 일관된 결과, 대량 데이터에서 성능 우위 |
| tsvector 관리 | schema.sql + 트리거 (JPA 엔티티 독립) | Hibernate DDL과 충돌 없이 DB 레벨에서 자동 갱신 |
